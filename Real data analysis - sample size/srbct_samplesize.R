...
# Load data
mydata <- readMat("Data/srbct.mat")
mydata <- cbind(as.data.frame(mydata$a$data), y = factor(mydata$a$nlab))
dim(mydata) # 83, 2309
table(mydata[, ncol(mydata)]) # 29, 11, 18, 25

# Response variable contains 4 categories >>> 1+2 vs 3+4
mydata[, ncol(mydata)] <- ifelse(mydata[, ncol(mydata)] == 1 | mydata[, ncol(mydata)] == 2, 0, 1)
table(mydata[, ncol(mydata)]) # 40, 43
...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    for (n in 1:nsamples) {
      ...
      C.init.less.std <- 1 # based on cv on whole dataset
      ...
      C.init.less.less <- 0.6309573 # based on cv on whole dataset
      ...
      C.init.less.lessstd <- 0.2511886 # based on cv on whole dataset
      ...
      C.init.less.lessstd2 <- 0.04365158 # based on cv on whole dataset
      ...
      C.init.svml1.none <- 0.6309573 # based on cv on whole dataset
      ...
      C.init.svml1.std <- 0.5754399 # based on cv on whole dataset
      ...
      C.init.svml1.less <- 0.724436 # based on cv on whole dataset
      ...
      C.init.svml1.lessstd <- 0.144544 # based on cv on whole dataset
      ...
      C.init.svml1.lessstd2 <- 2.290868 # based on cv on whole dataset
...
save(sim.list, file = "srbct_samplesize_simlist.RData")
...
write.csv(plots.mydata, "srbct_samplesize_results.csv", row.names = FALSE)
...
ggsave("srbct_samplesize_numbetas.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         # ggtitle("Number of selected variables for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
ggsave("other plots/srbct_samplesize_numbetas_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of number of selected variables for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
ggsave("srbct_samplesize_AUC.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Test AUC for increasing sample size (srbct data)") +
         xlab("Sample size") +
         ylab("AUC") +
         guides(color = guide_legend(order = 1, keywidth = 2.5),
                linetype = guide_legend(order = 2, keywidth = 2.5)) +
...
ggsave("other plots/srbct_samplesize_AUC_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of test AUC for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
ggsave("srbct_samplesize_accuracy.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Test accuracy for increasing sample size (srbct data)") +
         xlab("Sample size") +
         ylab("Accuracy (%)") +
         guides(color = guide_legend(order = 1, keywidth = 2.5),
                linetype = guide_legend(order = 2, keywidth = 2.5)) +
...
ggsave("other plots/srbct_samplesize_accuracy_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of test accuracy for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
ggsave("srbct_samplesize_generror.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Generalisation error for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
ggsave("other plots/srbct_samplesize_generror_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of generalisation error for increasing sample size (srbct data)") +
         xlab("Sample size") +
...
send_telegram_message(text = "The srbct script with increasing sample size is finished!",
                      chat_id = "441084295",
