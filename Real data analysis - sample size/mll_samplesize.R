...
# Load data
mydata <- readMat("Data/mll.mat")
mydata <- cbind(as.data.frame(mydata$a$data), y = factor(mydata$a$nlab))
dim(mydata) # 57, 5849
table(mydata[, ncol(mydata)]) # 20 ALL, 17 AML, 20 MLL

# Response variable contains 3 categories >>> 1+2 vs 3
mydata[, ncol(mydata)] <- ifelse(mydata[, ncol(mydata)] == 3, 0, 1)
table(mydata[, ncol(mydata)]) # 20, 37
...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    for (n in 1:nsamples) {
      ...
      C.init.less.std <- 1.905461 # based on cv on whole dataset
      ...
      C.init.less.less <- 0.3467369 # based on cv on whole dataset
      ...
      C.init.less.lessstd2 <- 0.6309573 # based on cv on whole dataset
      ...
      C.init.svml1.none <- 10.47129 # based on cv on whole dataset
      ...
      C.init.svml1.less <- 8.709636 # based on cv on whole dataset
      ...
      C.init.svml1.lessstd <- 0.05011872 # based on cv on whole dataset
      ...
      C.init.svml1.lessstd2 <- 6.91831 # based on cv on whole dataset
...
save(sim.list, file = "mll_samplesize_simlist.RData")
...
write.csv(plots.mydata, "mll_samplesize_results.csv", row.names = FALSE)
...
ggsave("mll_samplesize_numbetas.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         # ggtitle("Number of selected variables for increasing sample size (mll data)") +
         xlab("Sample size") +
...
ggsave("other plots/mll_samplesize_numbetas_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of number of selected variables for increasing sample size (mll data)") +
         xlab("Sample size") +
       ...
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Test AUC for increasing sample size (mll data)") +
         xlab("Sample size") +
         ylab("AUC") +
         guides(color = guide_legend(order = 1, keywidth = 2.5),
                linetype = guide_legend(order = 2, keywidth = 2.5)) +
...
ggsave("other plots/mll_samplesize_AUC_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of test AUC for increasing sample size (mll data)") +
         xlab("Sample size") +
...
ggsave("mll_samplesize_accuracy.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Test accuracy for increasing sample size (mll data)") +
         xlab("Sample size") +
         ylab("Accuracy (%)") +
         guides(color = guide_legend(order = 1, keywidth = 2.5),
                linetype = guide_legend(order = 2, keywidth = 2.5)) +
...
ggsave("other plots/mll_samplesize_accuracy_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of test accuracy for increasing sample size (mll data)") +
         xlab("Sample size") +
...
ggsave("mll_samplesize_generror.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Generalisation error for increasing sample size (mll data)") +
         xlab("Sample size") +
...
ggsave("other plots/mll_samplesize_generror_sd.png",
       ggplot(plots.mydata, aes(x = SampleSize)) +
         ...
         scale_linetype_manual(values = c("dotted", "solid", "dashed", "longdash", "twodash"),
                               labels = unname(TeX(c("$\\textit{x}$", "$\\textit{z}$", "$\\textit{\\mu_k}$", 
                                                     "$\\textit{\\mu_k \\sigma^2_k}$", 
                                                     "$\\textit{\\mu_k \\bar{\\sigma}^2}$")))) +
         # ggtitle("Standard deviation of generalisation error for increasing sample size (mll data)") +
         xlab("Sample size") +
...
send_telegram_message(text = "The mll script with increasing sample size is finished!",
                      chat_id = "441084295",
