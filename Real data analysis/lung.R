...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    # C.init.less.std <- 0.1778279 # based on cv on whole dataset
    ...
    # C.init.less.lessstd2 <- 0.05623413 # based on cv on whole dataset
    ...
    # C.init.svml1.none <- 3.162278 # based on cv on whole dataset
    ...
    # C.init.svml1.std <- 0.1 # based on cv on whole dataset
    ...
    # C.init.svml1.less <- 0.1 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd <- 0.01778279 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd2 <- 10 # based on cv on whole dataset
...
save(sim.list, file = "lung_simlist.RData")
...
write.csv(results.mydata, "lung.results.csv", row.names = FALSE)
...
write.csv(results.reps.mydata, "lung.results.reps.csv", row.names = FALSE)
...
send_telegram_photo(photo = "lung_numbetas.png",
                    caption = "Boxplot of the model dimensionality for the lung dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_photo(photo = "lung_auc.png",
                    caption = "Boxplot of the AUC values for the lung dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_message(text = "The lung script is finished!",
                      chat_id = "441084295",
                      bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")
