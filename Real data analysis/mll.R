...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    # C.init.less.less <- 1.778279 # based on cv on whole dataset
    ...
    # C.init.less.lessstd2 <- 0.5623413 # based on cv on whole dataset
    ...
    # C.init.svml1.none <- 1.778279 # based on cv on whole dataset
    ...
    # C.init.svml1.less <- 1 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd <- 0.03162278 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd2 <- 0.05623413 # based on cv on whole dataset
...
save(sim.list, file = "mll_simlist.RData")
...
write.csv(results.mydata, "mll.results.csv", row.names = FALSE)
...
write.csv(results.reps.mydata, "mll.results.reps.csv", row.names = FALSE)
...
send_telegram_photo(photo = "mll_numbetas.png",
                    caption = "Boxplot of the model dimensionality for the mll dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_photo(photo = "mll_auc.png",
                    caption = "Boxplot of the AUC values for the mll dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_message(text = "The mll script is finished!",
                      chat_id = "441084295",
                      bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")
