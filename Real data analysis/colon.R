...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    # C.init.less.std <- 1.778279 # based on cv on whole dataset
    ...
    # C.init.less.less <- 0.05623413 # based on cv on whole dataset
    ...
    # C.init.less.lessstd <- 0.01778279 # based on cv on whole dataset
    ...
    # C.init.svml1.none <- 0.05623413 # based on cv on whole dataset
    ...
    # C.init.svml1.less <- 0.03162278 # based on cv on whole dataset
...
save(sim.list, file = "colon_simlist.RData")
...
write.csv(results.mydata, "colon.results.csv", row.names = FALSE)
...
write.csv(results.reps.mydata, "colon.results.reps.csv", row.names = FALSE)
...
send_telegram_photo(photo = "colon_numbetas.png",
                    caption = "Boxplot of the model dimensionality for the colon dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_photo(photo = "colon_auc.png",
                    caption = "Boxplot of the AUC values for the colon dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_message(text = "The colon script is finished!",
                      chat_id = "441084295",
                      bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")
