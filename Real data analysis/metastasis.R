...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    # C.init.less.none <- 0.01 # based on cv on whole dataset
    ...
    # C.init.less.std <- 0.001778279 # based on cv on whole dataset
    ...
    # C.init.less.less <- 0.003162278 # based on cv on whole dataset
    ...
    # C.init.less.lessstd <- 0.003162278 # based on cv on whole dataset
    ...
    # C.init.less.lessstd2 <- 0.01778279 # based on cv on whole dataset
    ...
    # C.init.svml1.none <- 0.005623413 # based on cv on whole dataset
    ...
    # C.init.svml1.less <- 0.003162278 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd <- 3.162278 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd2 <- 5.623413 # based on cv on whole dataset
...
save(sim.list, file = "metas_simlist.RData")
...
write.csv(results.mydata, "metas.results.csv", row.names = FALSE)
...
write.csv(results.reps.mydata, "metas.results.reps.csv", row.names = FALSE)
...
send_telegram_photo(photo = "metas_numbetas.png",
                    caption = "Boxplot of the model dimensionality for the metas dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_photo(photo = "metas_auc.png",
                    caption = "Boxplot of the AUC values for the metas dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_message(text = "The metas script is finished!",
                      chat_id = "441084295",
                      bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")
