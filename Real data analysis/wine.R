...
sim.list <- foreach(k = 1:K) %dopar% {
  ...
  for (fold in 1:nfolds) {
    ...
    # C.init.less.std <- 0.3162278 # based on cv on whole dataset
    ...
    # C.init.less.less <- 1 # based on cv on whole dataset
    ...
    # C.init.less.lessstd <- 0.1 # based on cv on whole dataset
    ...
    # C.init.less.lessstd2 <- 1.778279 # based on cv on whole dataset
    ...
    # C.init.svml1.none <- 0.5623413 # based on cv on whole dataset
    ...
    # C.init.svml1.less <- 0.3162278 # based on cv on whole dataset
    ...
    # C.init.svml1.lessstd <- 1 # based on cv on whole dataset
...
save(sim.list, file = "wine_simlist.RData")
...
write.csv(results.mydata, "wine.results.csv", row.names = FALSE)
...
write.csv(results.reps.mydata, "wine.results.reps.csv", row.names = FALSE)
...
send_telegram_photo(photo = "wine_numbetas.png",
                    caption = "Boxplot of the model dimensionality for the wine dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_photo(photo = "wine_auc.png",
                    caption = "Boxplot of the AUC values for the wine dataset", 
                    chat_id = "441084295",
                    bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")

send_telegram_message(text = "The wine script is finished!",
                      chat_id = "441084295",
                      bot_token = "880903665:AAE_f0i_bQRXBXJ4IR5TEuTt5C05vvaTJ5w")
